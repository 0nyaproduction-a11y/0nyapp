...
async function run() {
  ...
  try{
    ...
    console.log("  url="+lu+" p="+lp.get("page")+" ps="+lp.get("pageSize")+" s="+lp.get("search")+" st="+lp.get("status"));
    await sh(pg,"c08b-08-list-query");
    console.log("Step 2: Open SF");
    const sfH=await fr(pg);
    if(sfH){await pg.goto(QA+sfH,{waitUntil:"networkidle",timeout:15000});console.log("  edit="+pg.url());}else{console.log("  row MISSING");}
    await sh(pg,"c08b-08-edit-bc");
    const bcH=await cb(pg);
    if(bcH)console.log("  bc="+bcH);else console.log("  bc MISSING");
    console.log("Step 3: BC->List");
    if(bcH){await pg.waitForURL(QA+"/admin/short-films",{timeout:15000});
      const ru=pg.url();const rp=new URL(ru).searchParams;
      const rP=rp.get("page"),rPS=rp.get("pageSize"),rSe=rp.get("search"),rSt=rp.get("status");
      console.log("  ret p="+rP+" ps="+rPS+" s="+rSe+" st="+rSt);
      qr=rP==="3"&&rPS==="50"&&(rSe===""||rSe===null)&&rSt==="draft";
      console.log("  QUERY_RESTORED: "+(qr?"PASS":"FAIL"));
    }else{console.log("  bc MISSING");}
    await sh(pg,"c08b-08-ret-list");
    console.log("Step 4: Dirty");
    await pg.goto(QA+"/admin/short-films?page=3&pageSize=50&search=&status=draft",{waitUntil:"networkidle"});
    await pg.waitForTimeout(1000);
    const sfH2=await fr(pg);
    if(sfH2){await pg.goto(QA+sfH2,{waitUntil:"networkidle",timeout:15000});console.log("  edit2="+pg.url());}
    else{console.log("  row2 MISSING");}
    const ti=await pg.$('input[name="title"]');let ot="";
    if(ti){ot=await rd(pg,"title");console.log("  orig="+ot);await fl(pg,"title",ot+" TESTEDIT");console.log("  edited");}
    else{console.log("  title MISSING");}
    const bc2=await cb(pg);
    if(bc2){try{await pg.waitForSelector('text=You have unsaved changes',{timeout:5000});dm=true;console.log("  dirty APPEARED");}catch(e){console.log("  dirty NOT");}}
    else{console.log("  bc2 MISSING");}
    await sh(pg,"c08b-08-dirty-modal");
    console.log("Step 5: Stay");
    if(dm){const sd=await ct(pg,"Stay");if(sd){await pg.waitForTimeout(1500);const su=pg.url();sk=su.match(QA+"/admin/short-films/")!==null;console.log("  editor="+(sk?"PASS":"FAIL"));const va=await rd(pg,"title");console.log("  val="+va);}else{console.log("  stay MISSING");}}
    else{console.log("  skip");}
    console.log("Step 6: Leave");
    const bc3=await cb(pg);
    if(bc3){try{await pg.waitForSelector('text=You have unsaved changes',{timeout:5000});console.log("  modal re-appeared");const lb=await ct(pg,"Leave without saving");if(lb){await pg.waitForURL(QA+"/admin/short-films",{timeout:15000});const lu2=pg.url();const lp2=new URL(lu2).searchParams;const a=lp2.get("page"),b2=lp2.get("pageSize"),c2=lp2.get("search"),d2=lp2.get("status");console.log("  leave p="+a+" ps="+b2+" s="+c2+" st="+d2);ln=true;lq=a==="3"&&b2==="50"&&(c2===""||c2===null)&&d2==="draft";console.log("  LEAVE_QUERY: "+(lq?"PASS":"FAIL"));}catch(e){console.log("  no modal");}}
    else{console.log("  bc3 MISSING");}
